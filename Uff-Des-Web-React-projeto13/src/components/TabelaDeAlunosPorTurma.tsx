...
const TabelaDeAlunosPorTurma = () => {
  ...
  const alunos = Array.isArray(resultado) ? resultado : resultado?.itens ?? [];
  ...
  const filtrados = useMemo(() => {
    ...
    if (termo)
      return arr.filter((a) => a?.nome?.toLowerCase()?.includes(termo));
    return arr;
  }, [alunos, pesquisa]);

  const totalDePaginas = Array.isArray(resultado)
    ? 1
